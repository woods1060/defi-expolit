...
const usdtAdddress = "0xdAC17F958D2ee523a2206206994597C13D831ec7";
...
describe("DODO_FlashLoan-attack-replay",function(){
    ...
    before(async function(){
        ...
        let token1 = await Token1.deploy("token1","t1",18,100000000000000000000,"CAPPED");
        let token2 = await Token2.deploy("token2","t2",18,100000000000000000000,"CAPPED");

        await token1.deployed();
        await token2.deployed();

        await token1.transfer(wallet,1000000000000000000);
        await token2.transfer(wallet,1000000000000000000);
    });
